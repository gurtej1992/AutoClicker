...
class Helper: NSObject {
    static let saveDefault = "saveTask"
    static let saveFileNotification = Notification.Name("saveFile")
    static let openFileNotification = Notification.Name("openFile")
    static func leftClick(with data:Task){
        let newY = NSHeight(NSScreen.screens.first!.frame) - CGFloat(data.yAxis)
        let source = CGEventSource.init(stateID: .hidSystemState)
        let position = CGPoint(x: CGFloat(data.xAxis), y: newY)
    ...
    static func rightClick(with data:Task){
        let newY = NSHeight(NSScreen.screens.first!.frame) - CGFloat(data.yAxis)
        let source = CGEventSource.init(stateID: .hidSystemState)
        let position = CGPoint(x: CGFloat(data.xAxis), y: newY)
    ...
    static func pressKey(with keyCode : UInt16){
    ...
    static func pressKeyComboWithCMD(with keyCode:UInt16){
        let src = CGEventSource(stateID: CGEventSourceStateID.hidSystemState)
    ...
    static func saveTasks(for array : [Task], at url : URL){
        let url = url.appendingPathExtension("plist")
    ...
    static func loadTask(at url : URL) -> [Task]?{
